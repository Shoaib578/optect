...
export default class ViewOrder extends React.Component{
     ...
     getData = async()=>{
        ...
        firestore().collection('assigned_operators').doc(this.props.route.params.id).onSnapshot(data=>{
            ...
            firestore().collection("orders").doc(data._data.order_id).onSnapshot(r=>{
                firestore().collection("status").where("operator_id","==",parse.id).where("order_id","==",r.id).get()
                .then(res=>{
  ...
  this.props.navigation.addListener("focus",()=>{
    ...
    render(){
        ...
        return(
            <View >
                <TouchableOpacity onPress={()=>this.props.navigation.navigate('add_status')} style={{left:'80%',marginTop:25}}>
                <FontAwesome name="plus" color="blue" size={20}/>
                </TouchableOpacity>
               ...
               {this.state.data.length>0?<ScrollView>
                ...
                {this.state.data.map((order,index)=>{
                    return   <TouchableOpacity onPress={()=>this.props.navigation.navigate('view_status',{status_id:order.id})} key={index} style={styles.OrderCard}>
              ...
              </ScrollView>:<Text style={{color:"red",fontWeight:"bold",marginTop:20,textAlign:"center"}}>{this.state.lang.dont_have_any}</Text>}
