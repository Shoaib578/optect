...
class AddSupervisor extends React.Component{
    ...
    pickImage =  () => {
     ...
     AddOperator = ()=>{
      ...
      getUserLang =async()=>{
        ...
        if(user_lang != null){
            if(user_lang == "spanish"){
                this.setState({user_lang: spn})
            }else{
                this.setState({user_lang: en})
    ...
    render(){
        return(
            ...
            <TouchableOpacity disabled={this.state.isLoading} onPress={this.AddOperator} style={styles.LoginButton}>
                {this.state.isLoading?<ActivityIndicator size="small" color="black"/>:null}
                    <Text style={styles.LoginButtonText}>{this.state.lang.add_operator}</Text>
            </TouchableOpacity>
                </View>
            </SafeAreaView>
            </ScrollView>
